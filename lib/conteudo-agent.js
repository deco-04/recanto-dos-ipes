...
const { makeRecentApprovedTitles, makeRecentRejectionsFeedback } = require('./content-history');
...
async function createWeeklyPackage(brand, propertyId, opts = {}) {
  ...
  // Weekly learning loop — two windowed reads feeding the prompt.
  const [recentPostTitles, recentFeedback] = await Promise.all([
    recentApprovedTitles({ brand, days: 30 }),
    recentRejectionsFeedback({ brand, days: 30 }),
  ]);

  const context = { recentPostTitles, recentFeedback };
  ...
  if (folderId && apiKey) {
    try {
      const driveClient = makeDriveImagesClient({ apiKey });
