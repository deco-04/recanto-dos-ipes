...
function startCronJobs() {
  ...
  // ── GHL Conversations poll — every 2 min ──────────────────────────────────
  // GHL has no "Outbound Message Sent" workflow trigger, so we poll the
  // Conversations API for active conversations (those with INBOUND in the
  // last 24h that have ever been linked to GHL via ghlMessageId) and mirror
  // any new OUTBOUND messages as InboxMessage(direction=OUTBOUND).
  // Auth failures (401) log a clear scope-missing warning but don't crash
  // the cron — the same code starts working as soon as the PIT gets the
  // required scopes.
  let _ghlConvPollRunning = false;
  cron.schedule('*/2 * * * *', async () => {
    if (_ghlConvPollRunning) return;
    const acquired = await tryAcquireLock('ghl-conv-poll', 5 * 60 * 1000);
    if (!acquired) return;
    _ghlConvPollRunning = true;
    try {
      await pollActiveGhlConversations();
    } catch (err) {
      console.error('[cron] GHL conversations poll failed:', err.message);
    } finally {
      _ghlConvPollRunning = false;
      await clearLock('ghl-conv-poll');
    }
  });

  console.log(`[cron] Jobs scheduled: iCal sync (hourly) · OTP cleanup (03:00) · Auto-complete past bookings (02:00) · Stale PENDING (30min) · Check-in today admin (08:00) · Daily briefing (10:00 UTC/07:00 BRT) · Task deadline (09:00) · Task overdue (10:00) · Inspection overdue (11:00) · Check-in welcome WA (11:00) · D-7 admin+guest (12:00) · Check-in guest reminder (13:00) · Survey push guest (15:00) · D-3 urgent guest-list (12:00 BRT) · Alert push (30min) · Survey score admin (2h) · Media retention (monthly 04:00, ${RETENTION_MONTHS}mo) · Content agent + RDI blog (Mon 14:00 UTC/07:00 MST) · SRI→RDS sync (04:00 BRT) · NPS reminder (11:00 BRT) · GHL social poll (hourly :07) · GHL conversations poll (every 2 min)`);
}
...
// ── GHL Conversations poll — extracted helper (testable) ─────────────────────

/**
 * Per-tick processor for the GHL Conversations 2-min poll.
 *
 * Walks the local conversations that look "active" (INBOUND in last 24h AND
 * at least one InboxMessage carrying a ghlMessageId — sign that GHL is wired
 * for this thread), then for each one:
 *   1. Calls GHL /conversations/search to find the matching ghlConvId by phone
 *   2. Calls GHL /conversations/:id/messages to fetch newest 20 messages
 *   3. Calls mirrorGhlMessage() per message — idempotent via ghlMessageId
 *
 * Auth-resilient: if /search returns 401 we log a single scope-missing
 * warning and stop the run. Other per-conversation errors are skipped so
 * one bad thread can't kill the whole loop.
 *
 * @param {object} [deps]
 * @param {object} [deps.ghlConv]      lib/ghl-conversations module (DI for tests)
 * @param {object} [deps.prismaClient] Prisma client (DI for tests)
 * @returns {Promise<{ polled: number, mirrored: number, authError?: boolean }>}
 */
async function pollActiveGhlConversations({ ghlConv, prismaClient = prisma } = {}) {
  const conv = ghlConv || require('./ghl-conversations');
  const { mirrorGhlMessage } = require('./ghl-conversations-mirror');

  const since = new Date(Date.now() - 24 * 60 * 60 * 1000);
  const activeConvs = await prismaClient.conversation.findMany({
    where: {
      lastMessageAt: { gte: since },
      // Only conversations that have ever been linked to GHL — keeps us from
      // fanning out search calls for legacy threads created before the hub.
      messages: { some: { ghlMessageId: { not: null } } },
    },
    select: {
      id:           true,
      contactPhone: true,
      messages: {
        orderBy: { sentAt: 'desc' },
        take: 1,
        select: { sentAt: true, ghlMessageId: true },
      },
    },
    take: 100,
  });

  if (activeConvs.length === 0) return { polled: 0, mirrored: 0 };
  ...
  for (const c of activeConvs) {
    ...
    const search = await conv.searchConversations({ limit: 20 });
    if (!search.ok) {
      if (!firstErrorLogged) {
        if (search.status === 401) {
          console.warn('[cron] GHL conversations poll: 401 — PIT missing scopes (conversations.readonly + conversations/message.readonly + contacts.readonly). Skipping until token has access.');
          authError = true;
        } else {
          console.error('[cron] GHL conversations poll: search failed:', search.error);
        }
        firstErrorLogged = true;
      }
      // No point continuing if auth is broken
      break;
    }

    const localTail = String(c.contactPhone || '').replace(/\D/g, '').slice(-11);
    ...
    if (!ghlConvObj?.id) continue;

    const messagesResult = await conv.getConversationMessages(ghlConvObj.id, { limit: 20 });
    if (!messagesResult.ok) continue;

    for (const m of messagesResult.messages) {
      try {
        const result = await mirrorGhlMessage({ ghlMessage: m, conversation: c, prismaClient });
        if (result.mirrored) mirroredCount++;
      } catch (e) {
        console.error(`[cron] GHL conversations poll: mirror failed for msg ${m?.id}:`, e.message);
      }
    }
  }

  if (mirroredCount > 0) {
    console.log(`[cron] GHL conversations poll: mirrored ${mirroredCount} new message(s) across ${activeConvs.length} active thread(s)`);
  }
  return { polled: activeConvs.length, mirrored: mirroredCount, ...(authError ? { authError: true } : {}) };
}
