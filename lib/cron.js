...
function startCronJobs() {
  ...
  cron.schedule('0 12 * * *', async () => {
    try {
      ...
      for (const booking of bookings) {
        ...
        // Push to GUARDIA and PISCINEIRO so they can plan ahead
        const d7Body = `${booking.guestName} · ${nights} noite${nights !== 1 ? 's' : ''} · Check-in ${checkInFmt}`;
        sendPushToRole('GUARDIA', {
        ...
        if (nights >= 5 && adminStaff) {
          ...
          if (!existing) {
            ...
            // WhatsApp to guest asking permission
            if (booking.guestPhone) {
              const waMsg = [
                ...
                ``,
                `Para estadias mais longas, nosso piscineiro costuma fazer uma visita de rotina para manter a piscina em perfeito estado durante a hospedagem. 🏊`,
                ``,
                `Você prefere que ele venha fazer a manutenção, ou prefere privacidade total durante a estadia?`,
                ``,
                `✅ *Sim* — pode vir, tudo bem!`,
                `🙏 *Não* — preferimos não receber visitas`,
                ``,
                `Qualquer resposta está ótima! 😊`,
              ].join('\n');

              sendWhatsAppMessage(booking.guestPhone, waMsg, booking.id).catch(e =>
                console.error('[cron] Pool WA message error:', e.message)
              );
            }
  ...
  cron.schedule('0 */2 * * *', async () => {
    try {
      ...
      for (const survey of newResponses) {
        ...
        await sendPushToRole('ADMIN', {
          title: isLow
            ? `⚠️ Avaliação baixa — ${score}★ de ${guestName}`
            : `Nova avaliação — ${score}★ de ${guestName}`,
          body:  survey.comment
            ? `${stars} · "${survey.comment.slice(0, 80)}"`
            : stars,
          type:  isLow ? 'SURVEY_LOW_SCORE' : 'SURVEY_HIGH_SCORE',
          data:  { surveyId: survey.id, bookingId: survey.bookingId, score },
        }).catch(() => {});
  ...
  console.log(`[cron] Jobs scheduled: iCal sync (hourly) · OTP cleanup (03:00) · Stale PENDING (30min) · Check-in today admin (08:00) · Task deadline (09:00) · Task overdue (10:00) · Inspection overdue (11:00) · D-7 admin+guest (12:00) · Check-in guest reminder (13:00) · Survey push guest (15:00) · Alert push (30min) · Survey score admin (2h) · Media retention (monthly 04:00, ${RETENTION_MONTHS}mo) · Content agent (Mon 10:00)`);
}
