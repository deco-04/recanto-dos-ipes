...
/**
 * Email service — Gmail API via OAuth2 (HTTPS, not SMTP).
 *
 * Railway blocks outbound SMTP ports (587/465). Gmail API uses port 443 (HTTPS)
 * which is always open. Reuses the same Google Cloud credentials already in use
 * for Google OAuth login (GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET).
 *
 * Required env vars:
 *   GMAIL_USER            — Gmail address, e.g. recantodoipes@gmail.com
 *   GOOGLE_CLIENT_ID      — already set in Railway (Google OAuth login)
 *   GOOGLE_CLIENT_SECRET  — already set in Railway (Google OAuth login)
 *   GMAIL_REFRESH_TOKEN   — one-time setup: run `node scripts/get-gmail-token.js`
 *
 * Gmail API quota: 500 recipients/day on free accounts — more than enough.
 */
