...
/**
 * Email service — uses Resend API (HTTPS, not SMTP).
 * Railway blocks outbound SMTP ports; Resend uses port 443 which is always open.
 *
 * Required env var: RESEND_API_KEY
 * Get one at: resend.com → free tier (3,000 emails/month)
 *
 * Sending domain: set EMAIL_FROM to match a domain you've verified in Resend.
 * For testing, Resend allows sending from onboarding@resend.dev to your own address.
 */
...
function getResend() {
  ...
  if (!process.env.RESEND_API_KEY) {
    throw new Error('[mailer] RESEND_API_KEY env var not set — email sending is disabled');
  }
...
const FROM = () => process.env.EMAIL_FROM || 'Recanto dos Ipês <onboarding@resend.dev>';
