...
const FROM_CDS      = () => `"Cabanas da Serra" <${process.env.GMAIL_USER}>`;
...
// ── CDS — Cabanas da Serra email templates ────────────────────────────────────

/**
 * Sent to guest when a CDS booking enters REQUESTED status.
 */
async function sendCDSBookingRequestReceived({ booking }) {
  const checkIn  = new Date(booking.checkIn).toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
  const checkOut = new Date(booking.checkOut).toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
  const total    = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(booking.totalAmount));
  const petLine  = booking.hasPet
    ? `<tr><td style="color:#6B6B6B;font-size:13px;">Pet</td><td style="color:#1A1A1A;font-weight:600;font-size:13px;">${escHtml(booking.petDescription) || 'Sim'}</td></tr>`
    : '';
  ...
  const html = `
    ...
    <body style="margin:0;padding:0;background:#F4F7F4;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#F4F7F4;padding:40px 20px;">
        <tr><td align="center">
          <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            <tr><td style="background:#2D5A4A;padding:32px;text-align:center;">
              <img src="https://cabanasdaserra.com/brand/cds-mark-white.svg" width="72" alt="Cabanas da Serra" style="display:block;margin:0 auto 20px;border:0;height:auto;">
              <p style="margin:0;color:#C4793A;font-size:22px;font-weight:700;">🏕️ Solicitação Recebida!</p>
              <p style="margin:8px 0 0;color:rgba(255,255,255,0.65);font-size:13px;">Cabanas da Serra · Jaboticatubas, MG</p>
            </td></tr>
            <tr><td style="padding:40px 36px;">
              <p style="margin:0 0 16px;color:#1A1A1A;font-size:16px;">Olá, <strong>${escHtml(booking.guestName)}</strong>!</p>
              <p style="margin:0 0 24px;color:#6B6B6B;font-size:14px;line-height:1.6;">
              ...
              <table width="100%" style="background:#F4F7F4;border-radius:12px;padding:20px;margin-bottom:24px;" cellpadding="8">
                <tr><td style="color:#6B6B6B;font-size:13px;">Check-in</td><td style="color:#1A1A1A;font-weight:600;font-size:13px;">${checkIn}</td></tr>
                <tr><td style="color:#6B6B6B;font-size:13px;">Check-out</td><td style="color:#1A1A1A;font-weight:600;font-size:13px;">${checkOut}</td></tr>
                <tr><td style="color:#6B6B6B;font-size:13px;">Hóspedes</td><td style="color:#1A1A1A;font-weight:600;font-size:13px;">${booking.guestCount}</td></tr>
                ${petLine}
                <tr><td style="color:#6B6B6B;font-size:13px;">Pré-autorizado</td><td style="color:#1A1A1A;font-weight:700;font-size:14px;">${total} <span style="font-weight:400;font-size:11px;color:#9A9A9A;">(não cobrado ainda)</span></td></tr>
              </table>
              <div style="background:#EEF5F2;border-radius:10px;padding:16px;margin-bottom:24px;">
                <p style="margin:0 0 8px;font-size:13px;font-weight:700;color:#2D5A4A;">O que acontece agora?</p>
                <p style="margin:0;font-size:13px;color:#2D5A4A;line-height:1.6;">
                  1. Nossa equipe revisa sua solicitação.<br>
                  2. Ao confirmar, o valor é cobrado e você recebe todos os detalhes.<br>
                  3. Se não pudermos confirmar, a pré-autorização é cancelada sem nenhum custo.
                </p>
              ...
              <p style="margin:0;color:#9A9A9A;font-size:12px;line-height:1.6;">
                Dúvidas? Acesse <a href="https://cabanasdaserra.com" style="color:#2D5A4A;">cabanasdaserra.com</a>
              </p>
            </td></tr>
            <tr><td style="background:#F4F7F4;padding:20px 36px;border-top:1px solid #D4E4DC;">
              <p style="margin:0;color:#9A9A9A;font-size:11px;text-align:center;">
                © ${new Date().getFullYear()} Cabanas da Serra · Jaboticatubas, MG
              </p>
            </td></tr>
          </table>
        </td></tr>
  ...
  await sendMail({
    ...
    subject: 'Sua solicitação de reserva foi recebida 🏕️',
    html,
    text: `Olá, ${booking.guestName}!\n\nRecebemos sua solicitação de reserva nas Cabanas da Serra para ${checkIn} a ${checkOut} (${booking.guestCount} hóspedes). Valor pré-autorizado: ${total}.\n\nNossa equipe confirmará em até 24 horas. Nenhum valor foi cobrado ainda.\n\ncabanasdaserra.com`,
  });
}

/**
 * Sent to guest when admin confirms a CDS booking.
 */
async function sendCDSBookingConfirmation({ booking }) {
  const checkIn  = new Date(booking.checkIn).toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
  const checkOut = new Date(booking.checkOut).toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
  const total    = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(booking.totalAmount));
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#F4F7F4;padding:40px 20px;">
        <tr><td align="center">
          <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            <tr><td style="background:#2D5A4A;padding:32px;text-align:center;">
              <img src="https://cabanasdaserra.com/brand/cds-mark-white.svg" width="72" alt="Cabanas da Serra" style="display:block;margin:0 auto 20px;border:0;height:auto;">
              <p style="margin:0;color:#C4793A;font-size:22px;font-weight:700;">🌿 Reserva Confirmada!</p>
              <p style="margin:8px 0 0;color:rgba(255,255,255,0.65);font-size:13px;">Cabanas da Serra · Jaboticatubas, MG</p>
            </td></tr>
            <tr><td style="padding:40px 36px;">
              <p style="margin:0 0 20px;color:#1A1A1A;font-size:16px;">Olá, <strong>${escHtml(booking.guestName)}</strong>!</p>
              <p style="margin:0 0 24px;color:#6B6B6B;font-size:14px;line-height:1.6;">
                Sua reserva foi confirmada! Estamos ansiosos para recebê-lo(a) nas Cabanas da Serra.
              </p>
              <table width="100%" style="background:#F4F7F4;border-radius:12px;padding:20px;margin-bottom:24px;" cellpadding="8">
                <tr><td style="color:#6B6B6B;font-size:13px;">Check-in</td><td style="color:#1A1A1A;font-weight:600;font-size:13px;">${checkIn}</td></tr>
                <tr><td style="color:#6B6B6B;font-size:13px;">Check-out</td><td style="color:#1A1A1A;font-weight:600;font-size:13px;">${checkOut}</td></tr>
                <tr><td style="color:#6B6B6B;font-size:13px;">Hóspedes</td><td style="color:#1A1A1A;font-weight:600;font-size:13px;">${booking.guestCount}</td></tr>
                ${booking.hasPet ? `<tr><td style="color:#6B6B6B;font-size:13px;">Pet</td><td style="color:#1A1A1A;font-weight:600;font-size:13px;">${escHtml(booking.petDescription) || 'Sim'}</td></tr>` : ''}
                <tr><td style="color:#6B6B6B;font-size:13px;">Total pago</td><td style="color:#1A1A1A;font-weight:700;font-size:14px;">${total}</td></tr>
                ${booking.invoiceNumber ? `<tr><td style="color:#6B6B6B;font-size:13px;">Reserva nº</td><td style="color:#1A1A1A;font-size:12px;font-family:monospace;">${booking.invoiceNumber}</td></tr>` : ''}
              </table>
              <p style="margin:0;color:#9A9A9A;font-size:12px;line-height:1.6;">
                Em caso de dúvidas, responda a este e-mail ou acesse <a href="https://cabanasdaserra.com" style="color:#2D5A4A;">cabanasdaserra.com</a>.
              </p>
            </td></tr>
            <tr><td style="background:#F4F7F4;padding:20px 36px;border-top:1px solid #D4E4DC;">
              <p style="margin:0;color:#9A9A9A;font-size:11px;text-align:center;">
  ...
  await sendMail({
    from:    FROM_CDS(),
    to:      booking.guestEmail,
    subject: `Reserva confirmada nas Cabanas da Serra — ${checkIn.split(',')[1]?.trim() || checkIn}`,
    html,
    text: `Olá, ${booking.guestName}!\n\nSua reserva nas Cabanas da Serra foi confirmada.\nCheck-in: ${checkIn}\nCheck-out: ${checkOut}\nTotal: ${total}${booking.invoiceNumber ? `\nReserva nº: ${booking.invoiceNumber}` : ''}`,
  });
}

/**
 * Sent to guest when admin declines a CDS booking.
 */
async function sendCDSBookingDeclined({ booking, declineReason }) {
  const checkIn  = new Date(booking.checkIn).toLocaleDateString('pt-BR', { day: 'numeric', month: 'long', year: 'numeric' });
  const checkOut = new Date(booking.checkOut).toLocaleDateString('pt-BR', { day: 'numeric', month: 'long' });
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#F4F7F4;padding:40px 20px;">
        <tr><td align="center">
          <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            <tr><td style="background:#2D5A4A;padding:32px;text-align:center;">
              <img src="https://cabanasdaserra.com/brand/cds-mark-white.svg" width="72" alt="Cabanas da Serra" style="display:block;margin:0 auto 20px;border:0;height:auto;">
              <p style="margin:0;color:#C4793A;font-size:18px;font-weight:700;">Atualização sobre sua solicitação</p>
              <p style="margin:8px 0 0;color:rgba(255,255,255,0.65);font-size:13px;">Cabanas da Serra · Jaboticatubas, MG</p>
            </td></tr>
            <tr><td style="padding:40px 36px;">
              <p style="margin:0 0 16px;color:#1A1A1A;font-size:16px;">Olá, <strong>${escHtml(booking.guestName)}</strong>,</p>
              ...
              <table cellpadding="0" cellspacing="0" style="margin:0 auto 24px;">
                <tr><td style="background:#2D5A4A;border-radius:10px;padding:12px 28px;">
                  <a href="https://cabanasdaserra.com" style="color:white;font-weight:700;font-size:14px;text-decoration:none;">Ver disponibilidade</a>
                </td></tr>
              </table>
              <p style="margin:0;color:#9A9A9A;font-size:12px;">
                Adoraríamos recebê-lo(a) em outra data!
              </p>
            </td></tr>
            <tr><td style="background:#F4F7F4;padding:20px 36px;border-top:1px solid #D4E4DC;">
              <p style="margin:0;color:#9A9A9A;font-size:11px;text-align:center;">
  ...
  await sendMail({
    from:    FROM_CDS(),
    to:      booking.guestEmail,
    subject: 'Atualização sobre sua solicitação — Cabanas da Serra',
    html,
    text: `Olá, ${booking.guestName},\n\nInfelizmente não foi possível confirmar sua solicitação.\n\nMotivo: ${declineReason || 'As datas solicitadas não estão disponíveis.'}\n\nA pré-autorização do seu cartão foi cancelada. Nenhum valor foi cobrado.\n\nConsulte outras datas em: cabanasdaserra.com`,
  });
}
