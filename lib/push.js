...
const { pushTypeToUrl } = require('./push-deeplinks');
...
async function sendPushToStaff(staffId, { title, body, type, data = {} }) {
  ...
  // Auto-inject `data.url` so the service worker
  // (`event.notification.data?.url || '/admin'`) routes the user to the
  // right in-app screen even if a caller forgot to set it. Existing
  // explicit `data.url` values win — see `pushTypeToUrl`.
  const resolvedUrl = pushTypeToUrl(type, data);
  const enrichedData = { ...data, url: resolvedUrl };

  try {
    await webpush.sendNotification(subscription, JSON.stringify({ title, body, type, data: enrichedData }));

    await prisma.pushNotification.create({
      data: { staffId, title, body, type, data: enrichedData },
    });
...
async function sendPushToUser(userId, { title, body, type, data = {} }) {
  ...
  // Auto-inject data.url for guest pushes too — same rationale as
  // sendPushToStaff. Guest types (CHECKIN_REMINDER, BOOKING_CONFIRMED_GUEST,
  // SURVEY_REQUEST, ...) usually carry their own data.url already; this
  // line is a safety net.
  const resolvedUrl = pushTypeToUrl(type, data);
  const enrichedData = { ...data, url: resolvedUrl };
