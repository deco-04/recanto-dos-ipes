...
async function syncIcal(source, url) {
  ...
  try {
    ...
    for (const r of reservations) {
      ...
      // ── Push notification to ADMIN: new OTA booking detected ─────────────
      const label = srcLabel(source);
      sendPushToRole('ADMIN', {
        title: `Nova reserva via ${label} 🏡`,
        body:  `${r.guestName} · ${fmtDate(r.checkIn)} → ${fmtDate(r.checkOut)} (${r.nights} noite${r.nights !== 1 ? 's' : ''}) — Toque para adicionar detalhes`,
        type:  'OTA_BOOKING_NEW',
        data:  { bookingId: booking.id, source },
      }).catch(e => console.error(`[ical-sync] push (new booking) error:`, e.message));
