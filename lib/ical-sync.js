...
async function syncIcal(source, url) {
  ...
  try {
    ...
    const existingOTABookings = await prisma.booking.findMany({
      ...
      select: { id: true, externalId: true, status: true, guestName: true, checkIn: true, checkOut: true, nights: true, lastCancellationNotifiedAt: true },
    });
    ...
    const oneDayAgo = new Date(Date.now() - CANCELLATION_NOTIFICATION_WINDOW_MS);

    // NOTE: The 24h dedupe window (CANCELLATION_NOTIFICATION_WINDOW_MS) means a booking
    // cancelled twice within 24h will only trigger one push notification. This is an
    // intentional trade-off: it prevents admin spam from transient feed issues, deploy
    // restarts, or status regression bugs. Re-cancellation within the window is rare in
    // normal operation.
    const cancelledBookings = existingOTABookings.filter(b =>
      b.status === 'CONFIRMED' &&
      !feedUids.has(b.externalId) &&
      new Date(b.checkOut) > now &&
      // Skip if we already notified about this cancellation within the dedupe window.
      // Treats null/undefined as "never notified" to allow first-time notification.
      (!b.lastCancellationNotifiedAt || b.lastCancellationNotifiedAt < oneDayAgo)
    );
