...
async function syncIcal(source, url) {
  ...
  try {
    ...
    const cancelledBookings = existingOTABookings.filter(b =>
      ...
      // Skip if we already notified about this cancellation within the last 24h
    ...
    for (const b of cancelledBookings) {
      await prisma.booking.update({
        ...
        data:  {
          status: 'CANCELLED',
          lastCancellationNotifiedAt: new Date(),
        },
      });
      bookingsCancelled++;
    }

    // Send a single batched push notification instead of one per booking
