...
async function syncIcal(source, url, propertyId) {
  ...
  try {
    ...
    // Resolve the property name once per sync. Used in the cancellation
    // push title so admins can tell at a glance WHICH property had a
    // cancellation — not just which OTA. Real-world reason this matters:
    // an admin running RDI + RDS + CDS sees identical "1 reserva
    // cancelada — Airbnb" notifications and can't act without opening
    // the app first. Reported 2026-04-30: "deveria mostrar o nome
    ...
    if (propertyId) {
      try {
        ...
        // Best-effort: if lookup fails the push falls back to source-only label.
      }
    }
    ...
    if (bookingsCancelled > 0) {
      ...
      // Title prefers the specific property name (e.g. "Sítio Recanto dos
      // Ipês · Airbnb") over the umbrella brand. Falls back to source-only
      // if propertyName couldn't be resolved (orphan booking, lookup error).
      const titleSuffix = propertyName ? `${propertyName} · ${label}` : label;

      let pushSucceeded = false;
      try {
        await sendPushToRole('ADMIN', {
          title: `${bookingsCancelled} reserva${bookingsCancelled > 1 ? 's' : ''} cancelada${bookingsCancelled > 1 ? 's' : ''} — ${titleSuffix}`,
          body:  bodyText,
          type:  'OTA_BOOKING_CANCELLED',
          // Include propertyId so the click handler can deeplink to the
          // right property's reservas screen instead of the umbrella view.
          data:  { source, count: bookingsCancelled, propertyId: propertyId || null, propertyName },
        });
        pushSucceeded = true;
      } catch (e) {
