 ...
 * - Upserts BlockedDate entries (keeps availability system working)
 * - Creates Booking records for real OTA reservations (dedup by externalId = UID)
 * - Fires GHL webhook for new bookings (non-blocking)
 *
...
async function syncIcal(source, url) {
  if (!url) {
    ...
    return { source, synced: 0, deleted: 0, bookings: 0, error: null };
  }
  ...
  try {
    ...
    // Delete dates no longer in the feed (cancellations)
    const existing = await prisma.blockedDate.findMany({
      where: { source },
      select: { id: true, date: true },
    });

    const toDelete = existing.filter(row => !dates.has(row.date.toISOString().split('T')[0]));
    ...
    // ── 2. Upsert Booking records for OTA reservations ────────────────────────
    let bookingsCreated = 0;

    for (const r of reservations) {
      if (!r.uid) continue;

      const existing = await prisma.booking.findUnique({ where: { externalId: r.uid } });

      if (existing) {
        // Update dates/name in case they changed
        await prisma.booking.update({
          where: { externalId: r.uid },
          data:  { guestName: r.guestName, checkIn: r.checkIn, checkOut: r.checkOut, nights: r.nights },
        });
        continue;
      }

      // Create new OTA booking record
      const booking = await prisma.booking.create({
        data: {
          ...
          status:          'CONFIRMED',
          source,
          invoiceNumber:   `${source.slice(0,3)}-${r.uid.slice(-8).toUpperCase()}`,
        },
      });

      bookingsCreated++;

      // Fire GHL webhook (non-blocking — do not await)
    ...
    if (bookingsCreated > 0) {
      console.log(`[ical-sync] ${source}: created ${bookingsCreated} new booking records`);
    }

    return { source, synced, deleted: toDelete.length, bookings: bookingsCreated, error: null };
  } catch (err) {
    console.error(`[ical-sync] ${source} sync failed:`, err.message);
    return { source, synced: 0, deleted: 0, bookings: 0, error: err.message };
  }
}
