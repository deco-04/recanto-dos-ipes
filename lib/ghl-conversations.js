 ...
 */
async function searchConversations({ locationId, limit = 50, lastMessageAfter = null, query = null } = {}) {
  const loc = locationId || process.env.GHL_COMPANY_ID;
  // GHL's /conversations/search has a hidden default filter that excludes
  // most threads (likely "starred-or-unread-only"). Without status=all the
  // endpoint returned 2 conversations when 3 actually existed, breaking
  // both the OUTBOUND mirror cron and the staff-send path's "find the GHL
  // conv by phone tail" lookup. Always pass status=all.
  ...
  // Optional server-side phone/name filter — much cheaper than fetching
  // every conversation client-side then filtering by phone tail.
...
/**
 * Create or find a GHL contact by phone (idempotent). Used by the staff
 * "Nova Conversa" flow when an admin manually adds a contact and starts a
 * WhatsApp thread. Phone must be E.164 with leading +.
 *
 * Returns the contact and a flag indicating whether it was newly created.
 *
 * GHL endpoint: POST /contacts/upsert — matches existing contacts by phone
 * (or email), returning the existing record without creating a duplicate.
 *
 * @param {object} args
 * @param {string} args.phone        E.164 phone with leading +
 * @param {string} [args.firstName]
 * @param {string} [args.lastName]
 * @param {string} [args.email]
 * @param {string} [args.locationId] defaults to GHL_COMPANY_ID env
 * @returns {Promise<{ ok: boolean, contactId?: string, isNew?: boolean, error?: string, status?: number }>}
 */
async function upsertContact({ phone, firstName, lastName, email, locationId } = {}) {
  const loc = locationId || process.env.GHL_COMPANY_ID;
  if (!loc) return { ok: false, status: 0, error: 'no-location-id' };
  if (!phone) return { ok: false, status: 400, error: 'phone-required' };

  const payload = { locationId: loc, phone };
  ...
  return {
    ...
    isNew:     Boolean(result.data?.new),
    contact:   result.data?.contact || null,
  };
}

module.exports = { searchConversations, getConversationMessages, sendMessage, upsertContact };
