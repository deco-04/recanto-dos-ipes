...
async function notifyBookingRequested(booking) {
  ...
  return postJson(url, payload)
    .then(() => console.log(`[ghl] notifyBookingRequested OK — booking ${booking.id.slice(-8)} (${booking.guestName})`))
    .catch(e => console.error('[ghl] notifyBookingRequested error:', e.message));
}
...
async function notifyBookingDeclined(booking) {
  ...
  return postJson(url, payload)
    .then(() => console.log(`[ghl] notifyBookingDeclined OK — booking ${booking.id.slice(-8)} (${booking.guestName})`))
    .catch(e => console.error('[ghl] notifyBookingDeclined error:', e.message));
}
