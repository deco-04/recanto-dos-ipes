...
| 4 | AGENDADO cards never auto-transition to PUBLICADO — GHL is canonical but we have no webhook/poller that reflects the state back | no file today (would need new webhook or cron) | Add GHL Social Planner webhook OR hourly poller | Large | Deferred |
